...
export default {
  name: 'TopicPage',
  head() {
    const title = `${this.topic.display} | Hong Kong Open Source Conference 2021`;
  ...
  computed: {
    ...
    twitterShareLink() {
      ...
      const text = `${this.speakers} in going to deliver ${this.topic.display} in HKOSCon 2021`;
      return `https://twitter.com/share?text=${encodeURIComponent(text)}&via=hkoscon&hashtags=${encodeURIComponent('hkoscon,hkoscon2021')}&url=${encodeURIComponent(shareUrl)}`;
    },
    linkedInShareLink() {
