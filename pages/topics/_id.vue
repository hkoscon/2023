...
export default {
  name: 'TopicPage',
  head() {
    const title = `${this.topic.display} | Hong Kong Open Source Conference 2023`;
  ...
  computed: {
    ...
    twitterShareLink() {
      ...
      const text = `${this.speakers} in going to deliver ${this.topic.display} in HKOSCon 2023`;
      return `https://twitter.com/share?text=${encodeURIComponent(text)}&via=hkoscon&hashtags=${encodeURIComponent('hkoscon,hkoscon2023')}&url=${encodeURIComponent(shareUrl)}`;
    },
    linkedInShareLink() {
